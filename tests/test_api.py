...
@pytest.mark.asyncio
async def test_signup_success(client: httpx.AsyncClient, db_session: AsyncSession):
...
@pytest.mark.asyncio
async def test_signup_existing_user_fails(client: httpx.AsyncClient, db_session: AsyncSession):
...
@pytest.mark.asyncio
async def test_login_success(client: httpx.AsyncClient, db_session: AsyncSession):
...
async def get_auth_token(client: httpx.AsyncClient, email: str, password: str):
    response = await client.post("/auth/login", data={
        "username": email,
        "password": password
    })
...
@pytest.mark.asyncio
async def test_create_ticket_success(client: httpx.AsyncClient, db_session: AsyncSession):
...
@pytest.mark.asyncio
async def test_get_ticket_success(client: httpx.AsyncClient, db_session: AsyncSession):
    """Test successful ticket retrieval"""
    # Create a test user and ticket directly in DB
    test_user = User(email="test@example.com", hashed_password=get_password_hash("testpass"), role="user")
...
@pytest.mark.asyncio
async def test_get_ticket_unauthorized(client: httpx.AsyncClient, db_session: AsyncSession):
    """Test user cannot access another user's ticket"""
    # Create two users
    user1 = User(email="user1@example.com", hashed_password=get_password_hash("hashedpass"), role="user")
    user2 = User(email="user2@example.com", hashed_password=get_password_hash("hashedpass"), role="user")
...
@pytest.mark.asyncio
async def test_reply_to_ticket_success(client: httpx.AsyncClient, db_session: AsyncSession):
    """Test agent can reply to ticket"""
    # Create user and agent
    user = User(email="ticketowner@example.com", hashed_password=get_password_hash("hashedpass"), role="user")
    agent = User(email="agent@example.com", hashed_password=get_password_hash("hashedpass"), role="agent")
...
@pytest.mark.asyncio
async def test_reply_to_ticket_non_agent_fails(client: httpx.AsyncClient, db_session: AsyncSession):
    """Test regular user cannot reply to tickets"""
    # Create user and ticket
    user = User(email="user@example.com", hashed_password=get_password_hash("hashedpass"), role="user")
    db_session.add(user)
    await db_session.commit()
    await db_session.refresh(user)
...
@pytest.mark.asyncio
async def test_update_ticket_status_success(client: httpx.AsyncClient, db_session: AsyncSession):
...
@pytest.mark.asyncio
async def test_list_tickets_user_sees_only_own(client: httpx.AsyncClient, db_session: AsyncSession):
    ...
    ticket1 = Ticket(title="User1 Ticket", description="Desc", status="open", created_by=user1.id)
    ticket2 = Ticket(title="User2 Ticket", description="Desc", status="open", created_by=user2.id)
...
@pytest.mark.asyncio
async def test_list_tickets_agent_sees_all(client: httpx.AsyncClient, db_session: AsyncSession):
    ...
    # Create tickets
    ticket1 = Ticket(title="Ticket 1", description="Desc", status="open", created_by=user1.id)
    ticket2 = Ticket(title="Ticket 2", description="Desc", status="open", created_by=user2.id)
    db_session.add_all([ticket1, ticket2])
    await db_session.commit()
